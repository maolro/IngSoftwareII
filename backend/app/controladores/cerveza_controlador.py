...
def api_buscar_cervezas():
    ...
    Usa query params: /cervezas/?q=...&estilo=...&pais=...
    """
    ...
    print(f"\n[DEBUG] Buscando con q={q}, estilo={estilo}, pais={pais}")
    
    cervezas = CervezaService.buscar_cervezas(g.db, q=q, estilo=estilo, pais=pais)
    print(f"[DEBUG] Cervezas encontradas: {len(cervezas)}")
    
    resultado = []
    for cerveza in cervezas:
        print(f"[DEBUG] Procesando cerveza ID: {cerveza.id}")
    ...
    print(f"[DEBUG] Devolviendo {len(resultado)} resultados.\n")
